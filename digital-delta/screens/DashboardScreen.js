...
export default function MapScreen() {
  // 1. Pull updateEdgeStatus from context
  const { nodes, edges, volunteers, updateEdgeStatus } = useData();
  ...
  // 2. Handle the message from WebView to update the global state
  ...
  const mapHtml = `
    ...
    <head>
      ...
      <style>
        ...
        .leaflet-popup-content-wrapper { background-color: ${colors.surface}; color: ${colors.text}; border-radius: 8px; }
        ...
        /* Custom Modal Styles (Matching your Route Screen) */
        #flood-modal {
      ...
      <script>
        ...
        graph.edges.forEach(edge => {
          ...
          if (sourceNode && targetNode) {
            const latlngs = [[sourceNode.lat, sourceNode.lng], [targetNode.lat, targetNode.lng]];
            ...
            // --- 5. ADD CLICK LISTENER TO EDGES ---
            polyline.on('click', function () {
              pendingEdge = edge;
              const text = edge.is_flooded 
        ...
        graph.nodes.forEach(node => {
          ...
          L.marker([node.lat, node.lng], { icon: customIcon }).addTo(map).bindPopup('<b>' + node.name + '</b>');
        });
        ...
        graph.volunteers.forEach(vol => {
          ...
          L.marker([vol.lat, vol.lng], { icon: customIcon }).addTo(map);
        });
      </script>
    </body>
    </html>
  ...
  return (
    <View style={styles.root}>
      ...
      <WebView
        ref={webviewRef}
        key={JSON.stringify(edges)} // 6. Force reload on edge updates
        originWhitelist={["*"]}
        source={{ html: mapHtml }}
        style={styles.map}
        scrollEnabled={false}
        onMessage={handleMessage} // 7. Attach the listener
      />
    </View>
  );
}
